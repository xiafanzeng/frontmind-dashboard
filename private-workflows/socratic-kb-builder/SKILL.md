...
`frontmind-kb-finalization-input-<digest>.zip` is a
server-owned system input, never a customer upload. Read
...
with exact provenance for those same bytes. An official-web Logo
requires `sourceKind: official_web` plus its real `sourcePageUrl` and direct
`sourceAssetUrl`; a Logo extracted from initial customer material requires
`sourceKind: official_document` plus the exact `sourceDocumentPath`. Never copy
the example domain or infer one source kind from company website availability.
When no Logo image is returned, omit `officialLogo` entirely.
...
5. Corrections, supplements, questions and customer uploads remain
   ...
   by the server-owned instructions/finalization ledger as `customerAttachments`
   or `customer_upload` are customer uploads and force supplement handling, even
   if the text says “确认”. Skill, instructions, prefill, evidence and
   finalization attachments are system inputs; they never change the server's
   declared action. A qualifying `official_logo_upload` resolves only the Logo
   block and still leaves the first leaf current for a later explicit
   confirmation.
