...
var serviceToken = process.env.FRONTMIND_PRESALES_SERVICE_TOKEN?.trim() || "";
var baseUrl = process.env.FRONTMIND_PRESALES_PROBE_BASE_URL?.trim() || `http://127.0.0.1:${process.env.PORT || "3001"}/api/internal/presales`;
var headers = {
...
async function errorCode(response) {
  try {
    ...
    return typeof body.error?.code === "string" ? body.error.code.slice(0, 80) : "UNSPECIFIED";
  } catch {
    return "NON_JSON_RESPONSE";
  }
}
async function expectOk(response, code) {
...
async function buildProbeArchive() {
  ...
  archive.file("frontmind-presales-roundtrip.txt", "frontmind\n", {
    date: /* @__PURE__ */ new Date("2026-01-01T00:00:00.000Z")
  });
...
try {
  ...
  const uploadTicket = typeof createdBody.proxy_upload_ticket === "string" ? createdBody.proxy_upload_ticket : "";
  ...
  if (downloaded.headers.get("content-type") !== "application/zip" || !downloaded.headers.get("content-disposition")?.includes(filename)) {
  ...
  console.error(
    JSON.stringify({
      ...
      httpStatus: error instanceof ProbeError ? error.status : void 0,
      cleanupStatus
    })
  );
  process.exit(1);
