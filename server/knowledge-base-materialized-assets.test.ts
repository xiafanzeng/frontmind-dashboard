...
async function revisedWorkingSet(
      ...
      add: [
        {
          ...
          sha256: sha256(replacementEvidence),
        },
      ],
      remove: ["evidence/1.1/product.md"],
    },
    assets: {
      add: [
        {
          ...
          sha256: sha256(replacementImage),
          mimeType: "image/png",
  ...
  return composeKnowledgeBaseWorkingSetRevision({ base, patch });
}
  ...
  return workingSet.manifest.leaves.map((leaf) => {
    ...
    return {
      ...
      sourceUrls: [
        ...
        ...workingSet.manifest.evidenceLedger
  ...
  it("projects only manifest-bound initial and revised local resources", async () => {
    ...
    expect(
      initialResources.every((resource) =>
        resource.sameOriginUrl.startsWith("/api/"),
      ),
    ).toBe(true);
    ...
    const evidenceUrl = initialResources.find(
      (resource) => resource.kind === "working_set_evidence",
    )!.sameOriginUrl;
    const pathSha256 = evidenceUrl.split("/").at(-2)!;
    ...
    expect(revisedResources.map((resource) => resource.filename)).toEqual([
      "product-revised.png",
      "revised.md",
    ]);
    expect(
      revisedResources.some((resource) => resource.filename === "product.png"),
    ).toBe(false);
    expect(() =>
      resolveKnowledgeBaseWorkingSetResource({
        buildId,
